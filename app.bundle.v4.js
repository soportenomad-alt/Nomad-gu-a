...
function initFirebase(){
  ...
  try{
    // Firebase compat SDK (cargado vía <script ...firebase-*-compat.js>)
    if (!window.firebase){
...
function renderRows(items){
  ...
  if (!items || !items.length){
    body.innerHTML = '<tr><td colspan="18" class="small">Sin datos.</td></tr>';
    return;
  }
...
const pacienteName = (it.paciente || (Array.isArray(it.pacientes) ? ((it.pacientes[0]?.paciente) || it.pacientes[0] || "") : (it.pacientes || "")) || "");
const folioGrupoDisp = (it.folioGrupo || "");
const folioDisp = (it.folio || "");
const pacienteDisplay = pacienteName;
const pacienteForModal = (folioDisp ? (folioDisp + " — " + pacienteDisplay) : pacienteDisplay);
const tr = document.createElement("tr");
    tr.innerHTML = `
      <td>${escapeHtml(createdAtText)}</td>
      <td>${escapeHtml(folioGrupoDisp)}</td>
      <td>${escapeHtml(folioDisp)}</td>
      ...
      <td>
        <button class="btn btn-mini btn-follow" type="button"
          ...
          data-paciente="${escapeHtml(pacienteForModal)}"
...
function subscribeRealtime(){
  ...
  unsubscribe = db.collection("solicitudes")
    .orderBy("createdAt", "desc")
    .limit(50)
    .onSnapshot((snap) => {
      const items = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      ...
      const body = document.getElementById("rtBody");
      if (body){
        body.innerHTML = '<tr><td colspan="12">Error leyendo Firestore. Revisa permisos (rules) y autenticación.</td></tr>';
      }
    });
}
...
async function logSolicitudIfPossible(payload){
  ...
  const pacientes = getPatientsWithTests();
  const pruebas = pacientes.map(p => p.prueba).filter(Boolean);
  const prueba = (pruebas.length && pruebas.every(x => x === pruebas[0])) ? pruebas[0] : (pruebas.length ? "Varios" : "");
  ...
  const folioGrupo = generateBaseFolio();
  const pacientesArr = Array.isArray(pacientes) ? pacientes : (pacientes ? [pacientes] : []);
  const total = pacientesArr.length || 0;

  // Guardamos 1 documento por paciente para que el seguimiento sea individual.
  // Usamos batch para escribir todo junto (más rápido y consistente).
  const batch = db.batch();

  for (let i = 0; i < pacientesArr.length; i++){
    const item = pacientesArr[i];

    const nombre = (typeof item === "string" ? item : (item?.paciente || item?.nombre || "")).trim();
    const pruebaItem = (typeof item === "string" ? "" : (item?.prueba || "")).trim();

    if (!nombre) continue;

    const folio = folioGrupo + "-" + String(i+1).padStart(2,"0");
...
function addPatient(value="", testValue="") {
  ...
  test.type = "text";
  test.placeholder = "Medicamento / Prueba";
  test.value = testValue;
  test.required = true;
  test.className = "patient-test";
...
function buildEmailPayload() {
  ...
  // Oculta el campo global de Prueba (se captura por paciente)
  const pruebaEl = document.getElementById("prueba");
  const pruebaField = pruebaEl ? pruebaEl.closest(".field") : null;
  if (pruebaField) pruebaField.style.display = "none";
  if (pruebaEl) pruebaEl.required = false;

  const kam = document.getElementById("kam").value.trim();
  const kamEmail = document.getElementById("kamEmail").value.trim();
  const medico = document.getElementById("medico").value.trim();

  const pacientesData = getPatientsWithTests();
  const pacientes = pacientesData.map(p => p.paciente);
  ...
  // Asunto: usa paciente (no médico). Si hay varios: primero +N
  const firstPaciente = pacientes[0] || "-";
  const extra = pacientes.length > 1 ? ` +${pacientes.length - 1}` : "";
  const subject = `Solicitud de recolección | ${firstPaciente}${extra} | ${hospital} | ${fecha}`;
  ...
  const lines = [
    ...
    "Medicamento/Prueba por paciente:",
    ...pacientesData.map(p => `- ${p.paciente}: ${p.prueba || "-"}`),
...
function validateRequired() {
  ...
  const pats = getPatientsWithTests();
  if (!pats.length) return false;

  // requiere nombre + medicamento/prueba por cada paciente
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Captura de Medicamento/Prueba es por paciente: ocultamos el campo global
  const pruebaEl = document.getElementById("prueba");
  const pruebaField = pruebaEl ? pruebaEl.closest(".field") : null;
  if (pruebaField) pruebaField.style.display = "none";
  if (pruebaEl) pruebaEl.required = false;
